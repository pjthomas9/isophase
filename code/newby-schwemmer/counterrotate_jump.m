...
for i = 1:N
	...
	temp(clockwise) = -(w -c*gamma*(r_all(i)^2-1)) / (2 * d_theta) + D / (r_all(i) * d_theta)^2;
	temp(counter_clockwise) = (w -c*gamma*(r_all(i)^2-1)) / (2 * d_theta) + D / (r_all(i) * d_theta)^2;
    
    L_dagger(i, :) = temp;
    disp(i/N*100)
end

T_bar = 6.283573419854775;

b = -ones(N, 1);
b(left_jump) = -1 + T_bar * ((w -c*gamma*(r_all(left_jump).^2-1)) / (2*d_theta) - D ./ (r_all(left_jump).^2 * d_theta^2));
b(right_jump) = -1 + T_bar * ((w -c*gamma*(r_all(right_jump).^2-1)) / (2*d_theta) + D ./ (r_all(right_jump).^2 * d_theta^2));
...
axis square
set(gca, 'FontSize', 20)
print('-depsc', 'counterrotate-surface')
...
colorbar
set(gca, 'FontSize', 20)
print('-depsc', 'counterrotate-contour')